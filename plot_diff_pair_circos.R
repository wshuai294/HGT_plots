...
pdf(file="diff_pair_matrix.pdf")
dat<-read.table("diff_pair_matrix.csv", header=TRUE, check.names = FALSE, sep=",")
...
grid.col <- rep(my_tem, times = 10)[1:73]
#grid.col<-"#6EE2FF"
arr <- read.csv('diff_pair_matrix_color.csv', header=FALSE)
grid.col <- arr
